...
class BarcodeScannerPlugin: Plugin, AVCaptureMetadataOutputObjectsDelegate {
  ...
  public override func load(webview: WKWebView) {
    ...
    // Use the system background color so the native layer matches the
    // user's light/dark appearance during the brief window before CSS loads.
    // Switched to .clear when the camera scanner is active.
