...
class BarcodeScannerPlugin(private val activity: Activity) : Plugin(activity),
    ...
    private var lastAnalyzerErrorMessage: String? = null
    private var lastAnalyzerErrorAtMs: Long = 0
    ...
    private fun logAnalyzerError(error: Throwable) {
        val message = error.message ?: error.toString()
        val now = System.currentTimeMillis()
        if (message != lastAnalyzerErrorMessage || now - lastAnalyzerErrorAtMs >= 3000L) {
            Logger.error(message)
            lastAnalyzerErrorMessage = message
    ...
    override fun analyze(image: ImageProxy) {
        ...
        activeScanner
            ...
            .addOnSuccessListener { barcodes ->
                if (barcodes.isNotEmpty() && savedInvoke != null) {
